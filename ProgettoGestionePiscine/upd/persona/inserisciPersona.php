        ...
        <?php
         ...
         $conn = pg_connect("host=localhost port=5432 dbname=GestionePiscine user=postgres password=postgres");
            ...
            if (!$conn) {
                ...
                $cfgenitore = isset($_POST['cfGenitore']) ? $_POST['cfGenitore'] : NULL;

                $query = "INSERT INTO persona (codice, nome, cognome, datanascita, cfgenitore)
                            VALUES ('$codice', '$nome','$cognome', '$datanascita', '$cfgenitore')";

                $result = pg_query($conn, $query);
                
                if($result){
                    echo("<br>Persona inserita correttamente!<br><br>");
                } else{
                    echo("<br>Persona NON inserito correttamente.<br><br>");
                }
